...
export const streamAnthropic: StreamFunction<"anthropic-messages", AnthropicOptions> = (
	...
	(async () => {
		...
		try {
			...
			type Block = (ThinkingContent | TextContent | (ToolCall & { partialJson: string }) | ServerToolUse | ServerToolResult) & { index: number };
			...
			for await (const event of anthropicStream) {
				if (event.type === "message_start") {
					...
					if (event.content_block.type === "text") {
						...
						// Amon-specific: handle server tool use (web search etc.)
					...
					if (block) {
						...
						if (block.type === "text") {
							...
							// Server tool result will follow as a separate block
						}
					}
...
function buildParams(
	...
	// Amon-specific: server tools (web search)
	if (options?.serverTools && options.serverTools.length > 0) {
		if (!params.tools) params.tools = [];
		for (const serverTool of options.serverTools) {
			(params.tools as any[]).push(serverTool);
		}
	}
